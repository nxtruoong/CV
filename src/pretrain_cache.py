"""One-shot JPEG -> uint8 memmap pre-decoder for SimCLR pretrain.

Eliminates per-step JPEG decode + PIL conversion from the dataloader hot loop.
~3-4x throughput win on Kaggle T4 x2 (4 vCPU) at batch 768.

Output:
  PRETRAIN_CACHE_PATH  = (N, R, R, 3) uint8 raw bytes (np.memmap)
  PRETRAIN_CACHE_INDEX = {"n": int, "res": int, "paths": [str, ...]}

Run once per Kaggle session before pretrain:
  python -m src.pretrain_cache

Re-runs idempotent: skips if output exists and (N, res) matches input listing.
"""
import argparse
import json
...
def _decode_one(args) -> int:
    ...
    # Reopen memmap per-worker for thread-safe file handles.
